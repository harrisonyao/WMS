...
<script>
	...
	var repositoryKVArray=new Array(); // 仓库信息及ID对照
		...
		$.ajax({
			...
			success : function(response){
				...
				$.each(response.rows,function(index,elem){
					repKVRow = new Array(2);
					repKVRow[0]= elem.id;
					repKVRow[1]= elem.address;
					repositoryKVArray.push(repKVRow);
					$('#search_input_repository').append("<option value='" + elem.id + "'>" + elem.address+"("+elem.id+")"+"号仓库</option>");
				})
			},
							...
							columns : [
									...
									},
									{
										...
										formatter : function (value,row,index) {
											var str = "";
											if (repositoryKVArray.length >0) {
												for(var i=0; i<repositoryKVArray.length;i++){
													var arr = repositoryKVArray[i];
													if(arr[0]==row.repositoryID){
														str = arr[1]+"("+arr[0]+")号仓库"
													}
												}
											}
